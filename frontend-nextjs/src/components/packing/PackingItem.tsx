...
export default function PackingItem({
  ...
  return (
    ...
    >
      ...
      {/* Category icon */}
      <span className="text-lg flex-shrink-0" title={locale === 'th' ? categoryConfig?.labelTh : categoryConfig?.labelEn}>
        {categoryConfig?.icon || '📦'}
      </span>
      ...
      <div className="flex-1 min-w-0">
        {isEditing ? (
          ...
          <div className="flex items-center gap-2">
            <span className={`truncate ${item.packed ? 'line-through text-gray-400' : ''}`}>
              {item.name}
            </span>
            {item.quantity > 1 && (
