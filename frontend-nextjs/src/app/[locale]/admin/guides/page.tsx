...
export default function AdminGuidesPage() {
    ...
    const fetchGuides = async () => {
        try {
            ...
            showError('Failed to load guides', err.message)
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleDelete = async () => {
        ...
        try {
            setDeleting(true)
            await adminService.deleteTrip(selectedGuide.id)
            showSuccess('Guide deleted successfully')
            setDeleteDialogOpen(false)
            setSelectedGuide(null)
            fetchGuides()
        } catch (err: any) {
            showError('Failed to delete guide', err.message)
        } finally {
            setDeleting(false)
        }
    }
    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                ...
                <p className="text-gray-500 mt-1">Manage travel guides</p>
            </div>
                ...
                <CardContent className="p-0">
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            <thead className="bg-gray-50 border-b">
                                <tr>
                                    <th className="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Guide</th>
                            ...
                            <tbody className="divide-y divide-gray-200">
                                {loading ? (
                                    ...
                                    data?.trips.map((guide) => (
                                        <tr key={guide.id} className="hover:bg-gray-50">
                                            <td className="px-6 py-4">
                                                <div className="font-medium text-gray-900">{guide.title}</div>
                                                <div className="text-sm text-gray-500">
                                                    {guide.destinations?.name}, {guide.destinations?.country}
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2">
                                                    {guide.owner?.photoUrl && (
                                                        <img
                                                            src={guide.owner.photoUrl}
                                                            alt={guide.owner.name}
                                                            className="h-6 w-6 rounded-full"
                                                        />
                                                    )}
                                                    <span className="text-sm text-gray-900">{guide.owner?.name || 'Unknown'}</span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    <Link href={`/${locale}/guides/${guide.id}`} target="_blank">
                    ...
                    {data && data.totalPages > 1 && (
                        <div className="flex items-center justify-between px-6 py-4 border-t">
                            <div className="text-sm text-gray-500">
                                Showing {((page - 1) * limit) + 1} to {Math.min(page * limit, data.total)} of {data.total} guides
                            </div>
